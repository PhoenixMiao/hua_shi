...
//        String result = HttpUtil.get("https://api.weixin.qq.com/sns/jscode2session", requestUrlParam);
    ...
    @Override
    public String resumeUpload(String userChuangNum, MultipartFile multipartFile, HttpServletRequest request){
        ...
        //拼接
        String name = userChuangNum+"." + firstName + lastName;
        ...
        try {
            ...
            User user=userMapper.selectOne(User.builder().chuangNum(userChuangNum).build());
            if(user.getAttachment1()==null)user.setAttachment1(filePath);
            else if(user.getAttachment2()==null)user.setAttachment1(filePath);
            else if(user.getAttachment3()==null)user.setAttachment3(filePath);
